...
- **This project's GCP org policy blocks making any Cloud Function/Cloud Run service publicly invokable at all** — confirmed by deploying a plain `onRequest` function (`viewReport`, since removed) and getting `Failed to set the IAM Policy on the Service` on every attempt, including via `invoker:"public"` and via a Firebase Hosting rewrite (Hosting's own rewrite-to-function proxy also got a 403, meaning even Firebase's internal service-to-service invocation is blocked, not just raw public access). If you need something reachable at a URL without the caller being logged into the ops app, don't reach for an HTTP-triggered Cloud Function here — instead build a static page under `public/` that reads Firestore directly via the client-side SDK with the same anonymous-auth pattern the main app uses (see `public/report.html` for a working example: reads `reports/{id}`, `document.write()`s the stored HTML). Firestore reads are governed by `firestore.rules`, not Cloud Run IAM, so they aren't subject to this restriction.
- **`fileClientContractToDrive` / `fileEmployeeAgreementToDrive` may not actually be running in production right now.** `firebase functions:list` shows both currently deployed with trigger type `https`, but the source (`onDocumentUpdated`) defines them as Firestore-triggered — deploying the current source fails immediately with "Changing from an HTTPS function to a background triggered function is not allowed." Function logs show a prior session already hit this, deleted the function to force a clean recreate, and the recreate then failed at the Cloud Build step ("missing permission on the build service account") — so the currently-live version is very likely a stale/orphaned deployment, not the Firestore-trigger logic described above. Treat the Drive auto-filing as unverified until someone confirms a real signed contract actually lands in Drive; the manual Download button is the only confirmed-working path in the meantime. Fixing this needs the Cloud Build service account permission resolved first (see the error's linked troubleshooting doc) before any redeploy of these two will succeed.
- There's also a `validateLogin` function (trigger type `callable`) live in this project that doesn't exist anywhere in this repo's source — likely leftover from an experiment deployed directly without being committed. Left alone for now; worth asking whoever deployed it before assuming it's safe to delete.
